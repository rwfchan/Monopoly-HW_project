...
Monopoly::GameState::GameState(const std::string& RulesFileName,
  ...
  //This is where the board was created
...
void Monopoly::GameState::playGame() {
  ...
  while (!isGameOver()) {
    ...
    do {
      ...
      //getCurrentPlayer().getSpaceOn().getSpaceNumber() == the jail space player is currently on
      Jail* jail = dynamic_cast<Jail*>(&this->board.getSpace(getCurrentPlayer().getSpaceOn().getSpaceNumber()));
      jail->Player_Move_in_jail(getCurrentPlayer());

      if(getCurrentPlayer().getJail_rupted()) {
        removeFromGame(getCurrentPlayer());
        break;
      }
...
void Monopoly::GameState::rollDice(Monopoly::Player& player) {
  ...
  //If player is in Jail, and the Jail sentence is not up yet, the player cannot move
  //Basically, if the number of jail sentence is anything but 0
  //-1 means player is not in jail
  //0 mean player's sentence is up
  if(player.getJail_sentence() > 0) {

    std::cout << player.getName() << ", you rolled a " << spacesToMove << std::endl;

    //If player get a double role, set player's remaining_sentence to 0
    //Release the player without taking money
    if(diceRoller.multiplesRolled()) {
      //FIXME Add line here???

      player.setRemaining_sentence(-1);
    } else {
      std::cout << "You are still stuck in jail" << std::endl;

      //Skip player's turn, player's sentence--
      player.sentence_countdown();
      return;
    }
  }
...
void Monopoly::GameState::buyUpgrade() {
  ...
  std::vector<Monopoly::Property*> upgradeableProperties = getCurrentPlayer().getUpgradeableProperties(getrule());
...
void Monopoly::GameState::sellUpgrade() {
  ...
  std::vector<Property*> downgradeableProperties = getCurrentPlayer().getDowngradeableProperties(getrule());
...
Monopoly::Rules Monopoly::GameState::getrule() const{
  return rules;
}

int Monopoly::GameState::getfree_parking_money() const {
  return free_parking_money;
}

void Monopoly::GameState::addfree_parking_money(int amount) {
  this->free_parking_money = free_parking_money + amount;
}

void Monopoly::GameState::resetfree_parking_money() {
